...
// FHIRサーバーから複数患者のデータを取得してServiceを初期化
func NewPatientServiceFromFHIRMultiple(baseURL string, count int) (*PatientService, error) {
	client := fhir.NewFHIRClient(baseURL)
	bundle, err := client.FetchMultiplePatientEverything(count)
	if err != nil {
		return nil, fmt.Errorf("failed to fetch from FHIR server: %w", err)
	}
	return &PatientService{bundle: bundle}, nil
